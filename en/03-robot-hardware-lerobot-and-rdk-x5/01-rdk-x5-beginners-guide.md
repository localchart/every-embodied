...
-->
# RDK X5 Super Beginner Tutorial: From System Burning to YOLO Object Recognition
...
The boards received will come with a box, which is very attractive:
...
RDK X5 provides functional interfaces such as network port, USB, camera, LCD, HDMI, CANFD, and 40PIN, which facilitate users in developing and testing applications like image multimedia and deep learning algorithms. The interface layout of the development board is as follows:
...
The RDK X5 development board uses a Micro SD storage card as the system boot medium. It is recommended to use a storage card with a capacity of at least `8GB`, so as to meet the storage requirements of the Ubuntu system and application functionality software.
...
When the system is first started, the default environment configuration is performed. The entire process takes about 45 seconds. After the configuration is completed, the Ubuntu system desktop will be displayed on the monitor, as shown in the following figure:
...
>
> If the output does not appear for a long time after the development board is powered on (more than 2 minutes), it means the board started abnormally. Debugging is required via the serial line to check if the board is functioning properly.
>
...
- **<font color='Blue'> does not power on upon startup </font>**: Please ensure that the adapter recommended in [ is used to supply power ](#供电); also, make sure the Micro SD card of the development board has been burned with a Ubuntu system image.
...
- `-w`: Width of saved images
- `-h`: Height of saved images
...
In this example, the YOLOv8 object detection algorithm is used to subscribe to images published by the MIPI camera. After algorithmic inference, an algorithm message is generated, and through the WebSocket package, the published images and corresponding algorithm results are rendered and displayed in a browser on a PC.
