...
- Run a smoke test for `64 个环境 x 5 次迭代` on Microduck RL before starting the formal training;
- Record simulation videos, export ONNX with normalized observations, and conduct pre-deployment testing in CPU MuJoCo;
- Determine what additional engineering steps are needed for a policy that performs well in simulation to achieve stable operation at 50 Hz on a real robot.
...
**Video 2: Official simulation of Open Duck Mini v2 walking segment.** This video is from the “Turning MuJoCo Playground” section of the Open Duck Mini v2 project. It corresponds to the old 42 cm robot, not to the 14-degree-of-freedom policy contract of the new Microduck.
...
The remaining observations come from the IMU, joint position/speed, and historical actions. The actor does not use the base linear velocity that is difficult to obtain reliably with a real robot; the critic can additionally read this simulation true value during training. This is a typical asymmetric actor-critic architecture: the value network uses more complete information to accelerate learning, while the policy network remains viable for deployment.
...
| :-- | :-- | :-- |
| Task Rewards | linear/angular velocity tracking | Track `vx, vy, yaw rate` command |
| Posture Rewards | upright, pose, head pose tracking | Avoid tipping over for speed while maintaining head control |
| Gait Rewards | air time, foot clearance, swing height | Encourage feet to truly leave the ground, preventing sliding on the ground |
...
Microduck RL does not rely on a perfectly accurate simulator. Instead, it does four things simultaneously:
...
Seeing tasks such as `Mjlab-Velocity-Flat-MicroDuck` and their successful testing only proves that dependencies, the registry, and configuration constraints are functional. However, this does not mean that GPU training can operate stably for a long time.
...
The experience provided in the official README is that available walking patterns can be seen in about 1–2 hours in a 4096 environment. However, the actual convergence time is affected by the GPU, current configuration, random seed, and curriculum. Do not treat this number as a hard guarantee.

At least the following should be monitored during a normal training session:

| Metric | What to Look At | Common Misjudgments |
| :-- | :-- | :-- |
| mean reward | Long-term trend is upward, not every iteration | Only looking at total reward, ignoring how the policy exploits rewards |
| episode length | Interpreted together with the current curriculum stage and fall rate | The longer, the better |
| velocity tracking | Both linear and angular velocities improve | Just moving forward is assumed to mean mastery of turning |
| penalty terms | The weighted penalty should be non-positive | The sign is reversed; the policy scores through self-collision behaviors |
| Motion and posture videos | Whether it scratches the ground, jumps with the buttocks, presses the head against the ground, or high-frequency shaking | Only looking at the curve, not the physical behavior |

The official command format for restoration after interruption is:
...
The video recording will be written in the `videos/play/` under the checkpoint log directory. It serves as evidence of the physical behavior of the gait, and cannot be replaced by a total reward curve.
...
In the Real Robot repository of Microduck `robotd` Running at 50 Hz, reading the IMU and servo status, generating a 61-dimensional observation, invoking the ONNX policy, and converting the 14-dimensional action into joint targets. But from `output.onnx` There must still be:

- Joint order, orientation, zero position, and limit verification;
...
| :-- | :-- | :-- |
| `uv sync` downloads CUDA wheel exceeds timeout | ARM requires downloading several GBs for the first time | Retry after `export UV_HTTP_TIMEOUT=600` |
| GPU not found during training start | Installed only CPU-only PyTorch, or CUDA/driver unavailable | Check `nvidia-smi` and `uv run python -c "import torch; print(torch.cuda.is_available())"` |
...
| Training reward increases but robot doesn’t move | Rewards exploit loopholes, such as shaking in place or local friction | Record and replay to check tracking, air-time, slip, and action-rate separately |
| Simulation replay works, but manual-exported ONNX fails | Observation normalizer not included in the computation graph | Export only using `scripts/export.py` |
| Policy exhibits high-frequency jitter on real robot | Action latency, joint order, zero position, or actuator dynamics mismatch | Return to CPU MuJoCo simulation and suspended small-scale testing; stop trial-and-error on the ground |
| Reference generator indicates STL storage representation error | Git LFS repository not pulled | Execute `git lfs pull` in the repository |
...
6. Only after passing the observation contract, joint calibration, and hardware safety mechanisms, proceed to use a real robot.
...
Both of the two videos and the header image in this section are from the above official project page. The video has only undergone format conversion, scaling, and segmenting, without altering the experimental content. The Open Duck Mini, Microduck, and Microduck RL repositories are released under the Apache-2.0 license; however, before use, you must refer to the current `LICENSE` and third-party assets of each repository.
